...
class _PistachioVpnScreenState extends State<PistachioVpnScreen>
  ...
  // continuous sway animation for the canopy
  ...
  Future<void> _handleToggle() async {
    if (_state == VpnState.disconnected) {
      ...
      setState(() => _state = VpnState.disconnecting);
      _stopTimer();
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          // swaying canopy with leaves + pistachios
          AnimatedBuilder(
            animation: _swayController,
            builder: (context, child) {
              final angle = (_swayController.value - 0.5) * 0.02; // small sway
...
/// Draws the static soil + trunk + branches.
...
/// Draws expanding, fading rings under the tree while connected.
...
/// A single pistachio nut that grows in when connected and falls away
/// when disconnecting, using implicit animations.
  ...
  Widget build(BuildContext context) {
    return Positioned(
      ...
      child: AnimatedSlide(
        ...
        offset: Offset(0, _dy / 24), // normalized to widget height ~24
